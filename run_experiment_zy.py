...
from experiments import experiments_zy as experiments
...
if (mode == 'all') or (mode == 'train'):
    ...
    p.add_argument('--deepReach_model', type=str, default='reg', required=False,
                   choices=['exact', 'exact_sin', 'exact_exp', 'exact_diff', 'reg', 'diff'], help='deepreach model')
    ...
    p.add_argument('--vsl_dt', type=float, default=0.01,
                   help='The dt used in rolling out trajectories to get consistancy labels')
    p.add_argument('--vsl_coef', type=float, default=500,
                   help='The weight for consistancy loss')
    p.add_argument('--num_vsl_samples', type=int, default=2000,
                   help='Number of consistancy samples')
    p.add_argument('--max_consistancy_steps', type=int, default=20,
                   help='Number of consistancy samples')
...
if (mode == 'all') or (mode == 'test'):
    ...
    p.add_argument('--data_step', type=str, default='run_basic_recovery', choices=['plot_violations', 'plot_hists', 'run_basic_recovery', 'plot_basic_recovery',
                   'collect_samples', 'train_binner', 'run_binned_recovery', 'plot_binned_recovery', 'plot_cost_function', 'run_robust_recovery',
                   'plot_robust_recovery'], help='The data processing step to run')
...
dataset = dataio.ReachabilityDataset(
    ...
    num_src_samples=orig_opt.num_src_samples, num_target_samples=orig_opt.num_target_samples,
     num_vsl_samples=orig_opt.num_vsl_samples, vsl_dt=orig_opt.vsl_dt)
...
experiment = experiment_class(
    model=model, dataset=dataset, experiment_dir=experiment_dir)
...
if (mode == 'all') or (mode == 'train'):
    if dynamics.loss_type == 'brt_hjivi':
        loss_fn = losses.init_brt_hjivi_loss(
            dynamics, orig_opt.minWith, orig_opt.dirichlet_loss_divisor, orig_opt.vsl_coef)
    ...
    experiment.train(
        ...
        use_consistancy_loss=orig_opt.use_consistancy_loss, max_consistancy_steps= orig_opt.max_consistancy_steps)
